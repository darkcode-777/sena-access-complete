...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($check->num_rows > 0) {
        echo "<script>alert('⚠️ Este aprendiz ya se encuentra registrado'); window.location.href='moto_portatil.html';</script>";
        exit;
    }
    ...
    $sql = "INSERT INTO aprendices 
        (nombre, apellido, correo, ficha, telefono, tipo_documento, documento, propiedad_moto, imagen_portatil, serial_portatil) 
        VALUES 
        ('$nombre', '$apellido', '$correo', '$ficha', '$telefono', '$tipo_documento', '$documento', '$propiedad_moto', '$imagen_portatil', '$serial_portatil')";

    if ($conn->query($sql) === TRUE) {
        echo "<script>alert('✅ Aprendiz registrado correctamente'); window.location.href='moto_portatil.html';</script>";
    } else {
